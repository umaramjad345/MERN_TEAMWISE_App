...
export const createTask = async (req, res) => {
  try {
    ...
    if (
      uploadedFileURLs.some((file) => !allowedFormats.includes(file.mimetype))
    ) {
      return res
        .status(400)
        .json({ status: false, message: "Image Format Not Supported" });
    }
    const publicUrls = [];
    for (const file of uploadedFileURLs) {
      try {
        ...
        publicUrls.push(cloudinaryResponse.secure_url);
      } catch (error) {
        console.error("Cloudinary Error:", error);
        return res
          .status(500)
          .json({ status: false, message: "Failed to Upload Images" });
      }
    }
...
export const updateTask = async (req, res) => {
  try {
    ...
    task.assets = publicUrls || task.assets;
    task.stage = (stage || task.stage).toLowerCase();
    task.team = teamArray || task.team;
